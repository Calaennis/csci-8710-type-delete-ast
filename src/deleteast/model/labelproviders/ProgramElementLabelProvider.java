...
import deleteast.util.UtilSearch;

/**
 * @since J2SE-1.8
 */
public class ProgramElementLabelProvider extends StyledCellLabelProvider {
   protected Text searchText;
   protected static Color colorYellow = Display.getCurrent().getSystemColor(SWT.COLOR_YELLOW);

   public ProgramElementLabelProvider(final Text searchText) {
      this.searchText = searchText;
   }

   public ProgramElementLabelProvider() {
   }
   ...
   public void update(ViewerCell cell) {
      ...
      if (search != null && search.length() > 0) {
         int intRangesCorrectSize[] = UtilSearch.getSearchTermOccurrences(search, cellText);
   ...
   protected String getCellText(ViewerCell cell) {
      ...
      String cellText = p.getPackageName();
      return cellText;
   }
}
